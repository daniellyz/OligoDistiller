...
#   ac_cmpd1, ac_cmpd2: list used by the BRAIN algorithm to compute theoretical isotope distributions. 
#                       The lists have to follow this format: list(C = 1, H = 2, F = 3, N = 4, O = 5, P = 6, S = 7)), where numbers 1-7 should be replaced with actual values
...
#   matching_mass_accuracy: numeric, relative (in ppm) mass tolerance for assigning observed peaks to theoretical peaks
#   noise_threshold: numeric, keep for the analysis only those peaks with intensities greater or equal than this threshold
#   extraction_mass_accuracy: numeric, 50 by default. mass tolerance (in ppm) for isotopic peak cluster extraction from the entire mass spectrum
...
#   proton_mass: 1.007276466621 Da

# output: list, with two elements:
#         by_charge: list, with elements like z5, z6, ..., each storing estimate (estimated proportion of cmpd2, 0-1 scale),
